...
class FirstOrderModel:
    ...
    def __init__(self, coeffs, system, dps=20):
        ...
        self._func['K6'] = lambda t: self._T1*self._T2*self._a1*self._a2*(self._T1 - self._T2 \
                                        ...
                                        + self._T2*exp(-(t-self._tau)/self._T2)) / (self._T1 - self._T2) #mp.fsub(self._T1, self._T2, dps=self._dps)
    ...
    def grid(self, scope):
        ...
        if (self.h(self._INF) - self.h(0)) == 0:
            #phia = lambda t: self._INF
            #phi = lambda t: self._INF
            ticks = np.arange(0., T+step, step)
            x = np.array([0 for t in ticks])
            y = np.array([0 for t in ticks])
            return (x, y)
        ...
        #phia = lambda t: mp.fdiv((mp.fsub(self.h(mp.fdiv(t, self._a, dps=self._dps)), self.h(0), dps=self._dps)), (mp.fsub(self.h(self._INF), self.h(0), dps=self._dps)), dps=self._dps)
        #phi = lambda t: mp.fdiv((mp.fsub(self.h(t), self.h(0), dps=self._dps)), (mp.fsub(self.h(self._INF), self.h(0), dps=self._dps)), dps=self._dps)


        X = lambda t: (phi(t) + phia(t)) / 2
        Y = lambda t: (phi(t) - phia(t)) / 2

        #X = lambda t: mp.fdiv(mp.fadd(phi(t), phia(t), dps=self._dps), 2, dps=self._dps)
        #Y = lambda t: mp.fdiv(mp.fsub(phi(t), phia(t), dps=self._dps), 2, dps=self._dps)
...
class StrejcModel:
    ...
    def __init__(self, coeffs, version):
        self._INF = 10**8 # discrete infinity
        self._a = 2.0
        ...
        self._factor = 1.0
        if version == 2:
            self._factor = self._tau / self._n
    ...
    def h(self, t):
        ...
        res = self._factor*self._k*(1 - res)
